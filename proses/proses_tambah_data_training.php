...
if (empty($kode_produk) || empty($nama_produk) || empty($tanggal) || empty($permintaan) || empty($persediaan) || empty($penjualan) || empty($produksi)) {
    ...
    if (!is_numeric($permintaan) || !is_numeric($persediaan) || !is_numeric($penjualan) || !is_numeric($produksi)) {
        ...
        $sql    = mysqli_query($conn, "INSERT INTO data_training (kode_produk, nama_produk, periode, permintaan, persediaan, penjualan, produksi)
        ...
        if ($sql) {
            ...
            echo "<script>alert('Data training gagal ditambah')</script>";
            echo "<script>window.location='../tambah_datatraining.php';</script>";
        }
    }
}
