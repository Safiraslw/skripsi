...
while ($r = mysqli_fetch_array($tampil_data)) {
    ...
    $update = mysqli_query($conn, "UPDATE tabel_hasil SET kode_produk='$kode_produk', nama_produk='$nama_produk', 
    periode='$tanggal', permintaan='$permintaan', persediaan='$persediaan', penjualan='$penjualan', hasil='$hasil' WHERE id_hasil='$id_hasil'");
    if ($update) {
        echo "<script>alert('Data prediksi berhasil diubah')</script>";
        echo "<script>window.location='../prediksi_bileh.php';</script>";
    } else {
        echo "<script>alert('Data prediksi tidak berhasil diubah')</script>";
